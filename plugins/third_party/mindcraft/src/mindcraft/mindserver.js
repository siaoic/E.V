...
export function createMindServer(host_public = false, port = 8080) {
    ...
    io.on('connection', (socket) => {
        ...
        socket.on('tts-playing', (agentName, playing) => {
            const agent = agent_connections[agentName];
            // agent 未登录（socket 为 null）时跳过：避免 null.emit 抛异常拖垮进程
            if (agent && agent.socket) {
                agent.socket.emit('tts-playing', playing);
            }
        });
    ...
    // 回环固定走 IPv4：Windows 下 'localhost' 会只绑 ::1（IPv6），
    // python-socketio 客户端走 IPv4 会被拒连（主项目 MindcraftBridge）。
