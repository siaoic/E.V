...
class MindServerProxy {
    ...
    async connect(name, port) {
        ...
        for (let attempt = 1; attempt <= maxRetries; attempt++) {
            try {
                this.socket = io(`http://localhost:${port}`, { reconnection: false });
                ...
                if (attempt >= maxRetries) throw new Error(`Failed to connect to MindServer after ${maxRetries} attempts`);
                console.log(`MindServer not ready, retrying in ${retryDelay/1000}s... (${attempt}/${maxRetries})`);
        ...
        this.socket.on('tts-playing', (playing) => {
            this.agent.self_prompter.setTTSPlaying(playing);
        });
