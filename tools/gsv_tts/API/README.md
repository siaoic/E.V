...
位置：gsv_tts/TTS.py
新增的异步方法：
infer_async() - 单个异步请求（线程安全）
infer_batched_async() - 批量异步请求（线程安全）
接口三：FastAPI 服务端 API
...
🎯 7. 最佳实践
服务端部署：使用 FastAPI 示例，提供 RESTful API
批量处理：尽量使用 infer_batched_async 提高 GPU 利用率
预热模型：在服务启动时预热模型，避免首次请求延迟
错误处理：在生产环境中添加适当的错误处理
端口管理：确保端口不被占用，必要时修改端口号
外链音频：确保 URL 可访问，建议使用 CDN 加速
🚀 8. 快速开始
最简单的使用方式（FastAPI）
# 1. 安装依赖
cd API
