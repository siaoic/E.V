...
- **Tell memU the uninstall happened — before the package can go.** The seams
  ...
  take `memu-agent` off this machine:

  ```
  memu-agent report uninstall
  ```
...
- **If the uninstall failed, or you are stopping early**, report that instead —
  ...
  ```
  memu-agent report error --stage uninstall --detail "<a full account of what went wrong>"
  ```
  ...
  **Be generous with `--detail`** — it is all a memU engineer gets to work out
  ...
  instead. If `pip uninstall` itself is what failed, `memu-agent` is usually still
