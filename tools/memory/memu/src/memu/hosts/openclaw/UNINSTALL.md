...
- **Tell memU the uninstall happened — before the package can go.** The seams
  ...
  take `memu-openclaw` off this machine:

  ```
  memu-openclaw report uninstall
  ```
...
- **If the uninstall failed, or you are stopping early**, report that instead —
  ...
  ```
  memu-openclaw report error --stage uninstall --detail "<a full account of what went wrong>"
  ```
  ...
  **Be generous with `--detail`** — it is all a memU engineer gets to work out
  ...
  instead. If `pip uninstall` itself is what failed, `memu-openclaw` is usually still
