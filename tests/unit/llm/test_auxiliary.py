...
class TestModelRouting:
    def test_default_uses_main_model(self, monkeypatch):
        ...
        monkeypatch.setattr("src.utils.config.cfg.llm.LLM_MODEL", "主模型A")
        monkeypatch.setattr("src.utils.config.cfg.llm.AUX_MODELS", {})
...
class TestCallLlm:
    def test_success_returns_text_and_usage(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            "src.llm.client.factory.get_async_openai_client",
            lambda **kw: fake)
    ...
    def test_no_model_fails_early(self, monkeypatch):
        monkeypatch.setattr("src.utils.config.cfg.llm.LLM_MODEL", "")
        monkeypatch.setattr("src.utils.config.cfg.llm.AUX_MODELS", {})
        text, usage = _run(auxiliary.call_llm("topic.gen", "你是助手", []))
        assert text is None
        assert usage["model"] == ""
