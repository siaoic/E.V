...
async def _run_chat_stream_inner(
    ...
    *,
    ...
    if tools:
        tool_block = (
            "### 工具\n"
            "需要实时信息/事实核查时，先调搜索或抓取工具拿真实结果再答，"
            "不要说自己无法联网。\n"
            "工具调用属内部细节：禁止向观众复述工具名/路径/JSON/执行过程；"
            "失败最多一句带过并重试，拿到结果直接给结论。\n"
            "乐谱图片先 read_sheet_music，再按返回的 score_path 调 "
            "play_score（.mid 才用 play_midi_file）；「再弹一遍」直接用最近"
            "一次 score_path；路径参数原样传递。"
        )
