...
from plugins.builtin.tools.sfx import _play_sound_effect as _core_play_sound_effect
...
async def _run_shell(sandbox: Sandbox, command: str) -> str:
    ...
    命令白名单校验 + 审计日志（ALLOW/DENY/TIMEOUT/ERROR 落盘 JSONL）。
    """
    if not sandbox.allow_shell:
        return "shell 未启用（AGENT_ALLOW_SHELL=false），操作被沙箱拒绝"
    if not command or not command.strip():
        return "命令为空"
    # 命令白名单校验（fail-closed：配置了白名单才启用）
    ok, reason = sandbox.check_command(command)
    if not ok:
        sandbox.audit("DENY", command, reason=reason)
        return f"操作被沙箱拒绝：{reason}"
    try:
        ...
        sandbox.audit("ALLOW", command, returncode=proc.returncode)
        return _fold_observation(out.decode("utf-8", errors="replace"), _SHELL_LIMIT)
    except asyncio.TimeoutError:
        proc.kill()
        sandbox.audit("TIMEOUT", command)
        return "命令执行超时（>30s），已终止"
    except OSError as e:
        sandbox.audit("ERROR", command, error=str(e))
        return f"命令执行失败：{e}"
