...
try:
    import socketio  # python-socketio（可选第三方依赖）
except Exception:  # pragma: no cover - 缺包时仅 MindcraftBridge.__init__ 抛错，导入链不中断
...
def _require_socketio() -> None:
    if socketio is None:
        raise RuntimeError(
            "MindcraftBridge 需要 python-socketio（与 aiohttp）第三方依赖。"
            " 请先 `pip install python-socketio aiohttp`，或在控制中心关闭 Mindcraft 插件。"
        )
