...
# 兼容别名：迁移前 src.danmaku.service 对外主类叫 DanmakuService，
# 拆分后实际「多房间弹幕服务」主实现是 BiliServiceManager。保留别名，
# 避免外部（注册页/控制中心）引用旧类名时 AttributeError。
DanmakuService = None  # 后向绑定，见底部
...
# 主类旧名兼容：把 BiliServiceManager 绑定给 DanmakuService。
# 外部 `from src.danmaku.service import DanmakuService` 仍能拿到真实主类。
DanmakuService = BiliServiceManager
