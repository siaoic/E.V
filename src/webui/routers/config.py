...
# TTS 参考音频上传目录：与 tts_service 共用，保证流式服务端可读取
TTS_AUDIO_UPLOAD_DIR = PROJECT_ROOT / "tts_service" / "refs"
_ALLOWED_TTS_AUDIO_EXT = {".wav", ".mp3", ".ogg", ".flac", ".m4a", ".aac", ".opus", ".wma"}
_MAX_TTS_AUDIO_BYTES = 50 * 1024 * 1024  # 50MB
...
@router.post("/tts-audio/upload")
async def upload_tts_audio(file: UploadFile = File(...)):
    """上传一份 TTS 参考音频，存到 tts_service/refs 并返回相对项目根的路径（保证目录可整体迁移）。"""
    ext = Path(file.filename or "").suffix.lower()
    if ext not in _ALLOWED_TTS_AUDIO_EXT:
        raise HTTPException(status_code=415, detail=f"不支持的音频格式: {ext or '空'}")
    data = await file.read(_MAX_TTS_AUDIO_BYTES + 1)
    if not data:
        raise HTTPException(status_code=400, detail="上传内容为空")
    if len(data) > _MAX_TTS_AUDIO_BYTES:
        raise HTTPException(status_code=413, detail=f"音频过大，不能超过 {_MAX_TTS_AUDIO_BYTES // (1024 * 1024)}MB")
    TTS_AUDIO_UPLOAD_DIR.mkdir(parents=True, exist_ok=True)
    dest = TTS_AUDIO_UPLOAD_DIR / f"upload_{uuid4().hex}{ext}"
    dest.write_bytes(data)
    # 返回相对项目根的路径（如 tts_service/refs/upload_xxx.wav），方便整目录迁移后仍能定位。
    return {"path": dest.relative_to(PROJECT_ROOT).as_posix(), "name": Path(file.filename or dest.name).name}


@router.get("/tts-audio-devices")
async def get_tts_audio_devices():
    """枚举本机可用于朗读（输出）的音频设备，供 WebUI 下拉选择扬声器。"""
    try:
        ...
        for index, dev in enumerate(devices):
            if dev.get("max_output_channels", 0) > 0:
                outputs.append({"index": index, "name": dev.get("name", f"设备 {index}")})
    except Exception as exc:
        logger.warning(f"枚举音频输出设备失败: {exc}")
        outputs = []
