...
class MainSystem:
    ...
    async def _init_components(self) -> None:
        ...
        try:
            ...
            for task in (plugin_runtime_task, a_memorix_task):
                if not task.done():
                    task.cancel()
        ...
        # W4 VTuber 口型桥（[vtuber].lipsync_enabled = false 时空转，零开销）
        from src.maisaka.vtuber_bridge.service import VtuberBridgeService

        await async_task_manager.add_task(VtuberBridgeService())
