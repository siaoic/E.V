...
class Messages(SQLModel, table=True):
    ...
    is_emoji: bool = Field(default=False)  # 已废弃：表情系统已停用，保留字段仅用于数据库兼容
...
class Images(SQLModel, table=True):
    ...
    image_type: ImageType = Field(sa_column=Column(SQLEnum(ImageType)), default=ImageType.IMAGE)
    """图片类型，表情系统已停用，仅保留 'image' 类型"""
