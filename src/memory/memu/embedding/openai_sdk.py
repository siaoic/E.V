...
class OpenAIEmbeddingSDKClient:
    ...
    def __init__(self, *, base_url: str, api_key: str, embed_model: str,
                 batch_size: int = 1, embed_dimensions: int | None = None):
    ...
    async def embed(self, inputs: list[str]) -> tuple[list[list[float]], CreateEmbeddingResponse | None]:
        ...
        if len(inputs) <= self.batch_size:
            ...
            response = await self.client.embeddings.create(model=self.embed_model, input=inputs, **kwargs)
        ...
        for i in range(0, len(inputs), self.batch_size):
            ...
            response = await self.client.embeddings.create(model=self.embed_model, input=batch, **kwargs)
