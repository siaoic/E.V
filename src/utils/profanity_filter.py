"""脏话过滤：加载 data/profanity.txt 大词库（每行一个词，7 万级）。
...
单字词条不参与匹配：profanity.txt 里的 70 个单字词（獨/慾/屍 等）子串匹配
会严重误伤正常弹幕（"独特" / "欲望" / "尸体"），过滤从 2 字词开始。

用法：
    filt = ProfanityFilter()         # 默认读 data/profanity.txt
...
class ProfanityFilter:
    ...
    def load(self, path: str) -> None:
        ...
        try:
            ...
            console.warn(f"[脏话过滤] 加载 {path} 失败：{e}（过滤未生效）")
            return
