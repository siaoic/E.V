...
from src.worlds.manager import WorldManager, get_world_manager
...
class MaisakaChatLoopService:
    ...
    def _build_chat_system_prompt(self, tools_section: str = "", *, include_world_env: bool = False) -> str:
        """基于当前配置实时构造主聊天系统提示词。"""

        return load_prompt(
            self._get_chat_prompt_name(),
            **self.build_prompt_template_context(tools_section, include_world_env=include_world_env),
        )
    ...
    def build_prompt_template_context(self, tools_section: str = "", *, include_world_env: bool = False) -> dict[str, str]:
        """构造 Maisaka prompt 模板的公共渲染参数。

        Args:
            tools_section: 额外注入到提示词中的工具说明片段。
            include_world_env: 是否包含世界清单段。世界清单「清单变化后只注入一次」，
                因此只有 planner 请求才应打开它，避免被其他用途的请求提前消费。
        """
        ...
        return {
            ...
            "worlds_section": self._build_worlds_env_section() if include_world_env else "",
        }
    ...
    def _get_bound_world_manager(self) -> Optional[WorldManager]:
        """取世界管理器；当前会话不是被绑定的直播聊天流时返回 None。"""

        manager = get_world_manager()
        if not self._session_id or self._session_id != manager.bound_session_id:
            return None
        return manager

    def _build_worlds_env_section(self) -> str:
        """构造世界清单段（``{worlds_section}``）；清单未变化时返回空串。"""

        manager = self._get_bound_world_manager()
        if manager is None:
            return ""
        return manager.build_env_injection()

    def _build_worlds_state_tail_message(self) -> str:
        """构造追加到请求末尾的世界状态消息；无待注入状态时返回空串。"""

        manager = self._get_bound_world_manager()
        if manager is None:
            return ""
        return manager.build_state_injection()
    ...
    def _build_request_messages(
        ...
        *,
        ...
        Args:
            ...
            include_world_context: 是否注入世界清单与世界状态，仅 planner 请求应为真。
        ...
        if system_prompt is not None:
            ...
            resolved_system_prompt = self._build_chat_system_prompt(
                include_world_env=include_world_context,
            )
        ...
        # 世界状态紧贴当前聊天注意事项之前注入；非 planner 请求不注入，
        # 否则会把待注入的 dirty 状态提前消费掉。
        worlds_state = self._build_worlds_state_tail_message() if include_world_context else ""
        final_user_messages = [
    ...
    async def chat_loop_step(
        ...
        *,
        ...
        built_messages = self._build_request_messages(
            ...
            include_world_context=request_kind == "planner",
        )
