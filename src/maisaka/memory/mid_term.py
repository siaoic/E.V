...
MID_TERM_MEMORY_RECALL_QUERY_EMBEDDING_TTL = 30.0
...
@dataclass(frozen=True, slots=True)
class MidTermMemoryRecallQueryCache:
    """一次聊天回想召回的 query 向量缓存，用于规避每 turn 重复的 embedding 往返。"""
...
# 聊天回想召回 query 向量缓存：key 为 session_id，TTL 内直接复用，避免每 turn 阻塞于 embedding 服务。
_mid_term_memory_recall_query_cache: dict[str, MidTermMemoryRecallQueryCache] = {}
...
async def build_mid_term_memory_reference_message(
    *,
    ...
    # 尽量复用 TTL 内已算好的 query 向量，规避每 turn 都阻塞于 embedding 的一次往返。
    cached_query = _mid_term_memory_recall_query_cache.get(session_id)
    if cached_query is not None and (monotonic() - cached_query.cached_at) <= MID_TERM_MEMORY_RECALL_QUERY_EMBEDDING_TTL:
        query_embedding = cached_query.embedding
        logger.debug(f"{log_prefix} 聊天回想召回命中 query 向量缓存，跳过 embedding")
    else:
        ...
        _mid_term_memory_recall_query_cache[session_id] = MidTermMemoryRecallQueryCache(
