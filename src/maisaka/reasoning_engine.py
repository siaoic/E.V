...
# 规则快速回复通道：文本长度不超过该值且为 @/提及触发的消息，跳过 Planner LLM 直接走 reply。
FAST_REPLY_MAX_CHARS = 40
...
class MaisakaReasoningEngine:
    ...
    async def _build_action_tool_definitions(self) -> tuple[list[dict[str, Any]], str]:
        ...
        for tool_spec in tool_specs:
            # 低延迟优化：直接把 wait 工具从本轮行动选项中跳过（无论 visible 还是 deferred），
            # 让 Planner 在决策时只能选择 reply 等真实行动，避免"该回却不回"的空转。
            if tool_spec.name == "wait":
                continue
        ...
        # 兜底：deferred 机制发现的 wait 也一并剔除，确保 planner 本轮永远拿不到 wait 工具。
        visible_tool_specs = [tool_spec for tool_spec in visible_tool_specs if tool_spec.name != "wait"]
    ...
    async def _try_rule_fast_reply(
        self,
        *,
        ...
        """规则快速回复通道：对简单必回消息跳过 Planner LLM，直接走 reply 生成正文。

        命中条件：消息为 @/提及触发、文本较短（无复杂记忆/工具诉求）。
        命中后省去一次 planner LLM 决策往返，显著降低端到端延迟；未命中则继续走完整 Planner。
        """

        plain_text = (trigger_message.processed_plain_text or "").strip()
        if not (trigger_message.is_at or trigger_message.is_mentioned):
            return False
        if not plain_text or len(plain_text) > FAST_REPLY_MAX_CHARS:
            return False

        started_at = time.time()
        self._runtime._update_stage_status("回复", "规则快速通道：直接生成回复")
        thought = "用户发送了一条简短消息，直接自然地回应一句即可。"
        fast_call = ToolCall(
            call_id=f"fast_reply_{uuid.uuid4().hex[:8]}",
            func_name="reply",
            args={"msg_id": trigger_message.message_id},
        )
        (
            should_pause,
            pause_tool_name,
            tool_result_summaries,
            tool_monitor_results,
        ) = await self._handle_tool_calls([fast_call], thought)
        state.planner_duration_ms = (time.time() - started_at) * 1000
        cycle_detail.time_records["planner"] = state.planner_duration_ms / 1000
        state.tool_result_summaries = tool_result_summaries
        state.tool_monitor_results = tool_monitor_results
        state.response = None
        state.cycle_end = (
            self._cycle_end_for_pause_tool(pause_tool_name)
            if should_pause
            else CycleEnd("fast_reply", "规则快速通道已直接完成回复。")
        )
        logger.info(
            f"{self._runtime.log_prefix} 规则快速通道：跳过 Planner LLM，直接回复（消息={plain_text[:24]!r}）"
        )
        return True
                ...
                try:
                    ...
                    while round_index < self._runtime._max_internal_rounds:
                        ...
                        try:
                            ...
                            if round_index == 0 and await self._try_rule_fast_reply(
