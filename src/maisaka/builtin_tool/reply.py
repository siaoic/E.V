...
from src.maisaka.live_text_filter import strip_live_visual_symbols
from src.maisaka.tts import tts_player
from src.maisaka.tts.live_player import LIVE_PLATFORM, live_voice_player
...
_RICH_REPLY_ARGUMENTS = {"attach_pic", "attach_at"}
...
def _use_expression_intent() -> bool:
    return (
        config_module.global_config.expression.expression_selection_mode
        == "vector_intent"
    )
...
def _require_hook_bool(raw_value: Any, option_name: str) -> bool:
    ...
    if isinstance(raw_value, bool):
        return raw_value
    raise TypeError(
        f"Hook 返回的 {option_name} 必须是布尔值，实际为 {type(raw_value).__name__}"
    )
...
async def _run_expression_selector(
    tool_ctx: BuiltinToolRuntimeContext, system_prompt: str
) -> str:
...
# 直播合并朗读时，段尾已是这些标点/符号则不再补停顿标点
_SEGMENT_ENDING_PUNCTUATION = "。！？!?；;，,、…—~～”』」)）]"


def _merge_live_speak_text(reply_segments: list[str]) -> str:
    """把分段回复合并为一条整段文本，供直播场景一次性朗读。

    逐段朗读会在段间等待下一次合成的首包，听感是频繁停顿；分句器还可能
    把英文昵称等切成孤段（如「Tiger」单独一段），整条合并后不存在这些问题。
    段与段之间补一个逗号作为朗读停顿（段尾已有标点时不补）。
    """

    cleaned = [
        segment.strip() for segment in reply_segments if segment and segment.strip()
    ]
    if not cleaned:
        return ""
    merged_parts: list[str] = []
    for index, segment in enumerate(cleaned):
        if index < len(cleaned) - 1 and segment[-1] not in _SEGMENT_ENDING_PUNCTUATION:
            segment += "，"
...
def _with_duplicate_target_reply_reminder(
    ...
    updated_args[_DUPLICATE_TARGET_REPLY_REMINDER_ARG] = (
        _DUPLICATE_TARGET_REPLY_REMINDER_TEMPLATE.format(
...
async def handle_tool(
    ...
    rich_reply_enabled = bool(
        config_module.global_config.experimental.enable_rich_reply
    )
    reply_tool_args = {
        key: value
    ...
    # 直播平台为纯文本通道，禁用图片与 at 等富媒体参数，避免媒体无法下发导致回复失败；
    # 用前缀匹配以兼容平台名带房间号/群后缀等派生形式。
    is_live = str(tool_ctx.runtime.chat_stream.platform or "").startswith(LIVE_PLATFORM)
    if is_live:
        for key in _RICH_REPLY_ARGUMENTS:
            reply_tool_args.pop(key, None)
            invocation_arguments.pop(key, None)
    if not _use_expression_intent():
        reply_tool_args.pop("expression_intent", None)
    enable_reply_quote = bool(
        config_module.global_config.chat.reply_style.enable_reply_quote
    )
    ...
    previous_target_reply = _find_recent_reply_to_target(
        replyer_chat_history, target_message_id
    )
    if previous_target_reply:
        reply_tool_args = _with_duplicate_target_reply_reminder(
            reply_tool_args, previous_target_reply
        )
    try:
    ...
    # 直播平台是纯文本弹幕通道：剔除 emoji 与颜文字，避免弹幕展示异常与 TTS 逐字朗读。
    if reply_text and is_live:
        reply_text = strip_live_visual_symbols(reply_text)
    ...
    try:
        ...
        logger.warning(
            f"{tool_ctx.runtime.log_prefix} 回复文本后处理前 Hook 调用失败，将使用默认策略: {exc}"
        )
        post_process_options = {
            "skip_post_process": False,
            "enable_splitter": True,
    ...
    reply_segments = [
        build_visible_text_from_sequence(sequence) for sequence in reply_sequences
    ]
    ...
    try:
        ...
        if tool_ctx.runtime.chat_stream.platform == CLI_PLATFORM_NAME:
            ...
            for index, reply_item in enumerate(reply_items):
                ...
                segment_set_quote, segment_reply_message = (
                    _resolve_segment_reply_context(
                        index=index,
                ...
                sent_message = await send_service._send_to_target_with_message(
                    ...
                    selected_expressions=(
                        None
                        if is_live
                        else (reply_result.selected_expression_ids or None)
                    ),
                    typing=index > 0,
                ...
                # 流式朗读本段回复文本：非阻塞入队。
                # 直播聊天流走独立直播语音路径（主播扬声器/直播间采集），
                # 其余平台走通用本机朗读，避免直播回复被播放两遍造成回声。
                # 直播回复不在此逐段朗读：发送完成后整条合并朗读（见下方）。
                if tool_ctx.runtime.chat_stream.platform != LIVE_PLATFORM:
                    await tts_player.speak(segment)
                ...
                if not sent:
                    ...
                    break
                sent_message_id = str(
                    getattr(sent_message, "message_id", "") or ""
                ).strip()
                if sent_message_id:
        ...
        # 直播场景整条合并朗读：逐段朗读会在段间等待合成首包造成停顿，
        # 且分句器可能把英文昵称等切成孤段，因此所有段发送完成后单次入队
        if sent and tool_ctx.runtime.chat_stream.platform == LIVE_PLATFORM:
            merged_speak_text = _merge_live_speak_text(reply_segments)
            if merged_speak_text:
                await live_voice_player.speak(merged_speak_text)
    except Exception:
        logger.exception(
