...
from src.maisaka.vtuber_bridge.emote import EmoteToolProvider
...
from src.worlds.tool_provider import WorldToolProvider
from src.worlds.types import WorldEventTrigger
...
class MaisakaHeartFlowChatting(MaisakaFocusRuntimeMixin, MaisakaRuntimeDisplayMixin):
    ...
    async def register_message(self, message: SessionMessage) -> None:
        ...
        self._ingest_cached_message(message)
        if self._is_reply_effect_tracking_enabled():
    ...
    def _ingest_cached_message(self, message: SessionMessage) -> None:
        """把消息并入待处理缓存并同步监控面板。

        这是「消息进入待处理缓存」的公共尾部：普通消息与世界事件都走这里，
        差别只在于前面的统计与触发处理。
        """
    ...
    def register_world_event(self, *, source_name: str, text: str, trigger: str) -> None:
        """缓存一条世界事件并按投递语义调度；不污染外部消息统计。

        世界事件（游戏状态变化、观众礼物、语音转写）不是聊天用户，因此这里刻意
        跳过 ``_record_external_message_interval`` 与 ``_update_message_trigger_state``：
        前者会把事件算成外部用户消息、扰动空窗补偿与空闲退避，后者会因事件文本
        恰好含 bot 昵称而意外绕过频率门控。

        Args:
            source_name: 事件来源显示名（例如「世界·B站直播间」），作为发言者前缀。
            text: 已渲染的中文事件文本。
            trigger: 投递语义，取值为 :class:`WorldEventTrigger` 的字符串值。
        """
        normalized_text = str(text or "").strip()
        if not normalized_text:
            raise ValueError("世界事件缺少正文")
        try:
            normalized_trigger = WorldEventTrigger(str(trigger or "").strip().lower())
        except ValueError as exc:
            allowed = "、".join(item.value for item in WorldEventTrigger)
            raise ValueError(f"世界事件的 trigger 取值非法: {trigger!r}，可选：{allowed}") from exc

        if self._running:
            self._ensure_background_tasks_running()

        message = self._build_world_event_message(source_name=source_name, text=normalized_text)
        self._ingest_cached_message(message)

        if normalized_trigger is WorldEventTrigger.PREEMPT:
            if self._agent_state == self._STATE_RUNNING:
                self._mark_message_debounce_required()
            self._request_planner_interrupt_for_message(message)
            if self._running:
                self._schedule_message_turn()
        elif normalized_trigger is WorldEventTrigger.FLUSH:
            # 绕过频率阈值，立即进入 Planner
            self._arm_forced_turn_state(message_id=message.message_id, reason="世界事件")
            self._idle_backoff.reset()
            if self._running:
                self._schedule_message_turn()
        elif normalized_trigger is WorldEventTrigger.DEBOUNCE:
            if self._running:
                self._schedule_message_turn()
        # PIGGYBACK：只写缓存，随其他批次投递

    def _build_world_event_message(self, *, source_name: str, text: str) -> SessionMessage:
        """构造世界事件消息；发言者前缀用世界显示名，正文即事件文本。"""
        message_id = f"world_event:{int(time.time() * 1000)}:{len(self.message_cache)}"
        message = SessionMessage(
        ...
        message.session_id = self.session_id
        message.message_info = MessageInfo(
            user_info=UserInfo(user_id=f"world:{source_name}", user_nickname=source_name, user_cardname=None),
            group_info=self._build_group_info(),
            additional_config={},
        )
    ...
    def _register_tool_providers(self) -> None:
        ...
        self._tool_registry.register_provider(WorldToolProvider())
        # W4 表情工具：与 WorldToolProvider 同层的独立 Provider（§4-D7），
        # 未启用/未连接时 list_tools 返回空，注册本身零开销
        self._tool_registry.register_provider(EmoteToolProvider())
