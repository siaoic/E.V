...
class ChatReplyStyleConfig(ConfigBase):
    ...
    group_chat_prompt: str = Field(
        default=(
            "你正在群里聊天，下面是群里正在聊的内容，聊天中包含文字、图片等消息。\n"
            "回复尽量简短一些。最好一次对一个话题进行回复，但必须考虑不同群友发言之间的交互，免得啰嗦或者回复内容太乱。请注意把握聊天内容。\n"
            "不要总是提及自己的身份背景，根据聊天内容自由发挥，但是要日常不浮夸，不要刻意找话题。\n"
            "不要输出表情包、emoji 或颜文字，只用纯文字表达。控制回复的频率，不要每个人的消息都回复，优先回复你感兴趣的或者主动提及你的，适当回复其他话题。\n"
        ),
        json_schema_extra={
            "label": {
                "zh_CN": "群聊提示词",
...
class TtsReadConfig(ConfigBase):
    """AI 语音朗读配置类"""

    __ui_label__ = "语音朗读"
    __ui_icon__ = "volume-2"

    enable_tts_read: bool = Field(
        default=False,
        json_schema_extra={
            "label": {
                "zh_CN": "朗读 AI 回复",
                "en_US": "Read aloud AI replies",
                "ja_JP": "AI返信を読み上げ",
            },
            "x-widget": "switch",
            "x-row": "tts-master-row",
        },
    )
    """开启后，麦麦对外发送的文本回复会交给独立的 TTS 服务流式合成，并播放到本机指定（虚拟）扬声器；不会发回直播间。具体服务地址、音色与输出设备等详参 tts_config.toml。"""
    ...
    output_device: str = Field(
        ...
        json_schema_extra={
            ...
            "x-widget": "device-select",
            "x-row": "tts-master-row",
        },
    )
    "朗读时使用的音频输出设备（如虚拟扬声器 VB-CABLE 等）。留空表示使用系统默认设备。"
    ...
    spk_audio: str = Field(
        ...
        json_schema_extra={
            "label": {
                "zh_CN": "参考音频文件",
                "en_US": "Speaker reference audio",
                "ja_JP": "話者参照音声",
            },
            "x-widget": "upload",
        },
    )
    "主参考音频（决定 AI 的音色与说话风格）。需要配合下面的「参考音频文本」使用；留空表示使用服务端默认。"

    spk_audio_text: str = Field(
        default="",
        json_schema_extra={
            "label": {
                "zh_CN": "参考音频文本",
                "en_US": "Reference audio transcript",
                "ja_JP": "話者参照音声の文字起こし",
            },
            "x-widget": "textarea",
            "placeholder": {
                "zh_CN": "参考音频里朗读的内容，用于对齐语气；留空则使用服务端默认",
                "en_US": "What the reference audio says; empty uses server default",
                "ja_JP": "参照音声の内容。空欄はサーバー既定",
            },
        },
    )
    "参考音频里朗读的内容，帮助模型对齐语气与情感。留空时自动使用服务端默认文本。"

    spk_audio_additional: List[str] = Field(
        default_factory=list,
        json_schema_extra={
            "label": {
                "zh_CN": "辅助参考音频",
                "en_US": "Additional reference audios",
                "ja_JP": "追加の話者参照音声",
            },
            "x-widget": "upload",
        },
    )
    "额外的音色参考音频，可添加多个、无需填文本，用于加强音色克隆；留空表示不添加。以等权重与主参考音频融合。"
...
class ChatConfig(ConfigBase):
    ...
    tts_read: TtsReadConfig = Field(default_factory=TtsReadConfig)
    """AI 语音朗读配置。"""
...
class ExperimentalConfig(ConfigBase):
    ...
    """开启后，reply 动作可通过 attach_pic、attach_at 参数附加图片或 at。"""
...
class CommandPermissionConfig(ConfigBase):
    ...
    """允许执行命令的用户，格式如 platform:user_id。"""
...
class PluginConfig(ConfigBase):
    ...
    permission: list[str] = Field(
        ...
        json_schema_extra={
            ...
            "x-widget": "tags",
            "x-placeholder": "platform:user_id",
        },
    )
    """允许用聊天命令管理插件的用户，格式如 platform:user_id。"""
...
class WorldsConfig(ConfigBase):
    """世界（事件驱动多 World）基座配置类

    基座本身在主程序内（``src/worlds/``），各具体世界（Minecraft / PvZ /
    Bilibili / ASR…）由插件实现并在加载时注册进来。默认关闭，保证不改变
    现有直播聊天行为。
    """
    ...
    enabled: bool = Field(
        ...
        json_schema_extra={
            "label": {
                "zh_CN": "启用世界框架",
                "en_US": "Enable world framework",
                "ja_JP": "ワールドフレームワークを有効化",
            },
            "x-widget": "switch",
        },
    )
    """开启后，世界事件与世界状态才会进入直播聊天流的决策上下文。"""

    live_platform: str = Field(
        default="bilibili_live",
        json_schema_extra={
            "label": {
                "zh_CN": "直播平台",
                "en_US": "Live platform",
    ...
    """直播聊天流所属平台；未显式指定 stream_id 时据此自动定位直播聊天流。"""
    ...
    stream_id: str = Field(
        ...
        json_schema_extra={
            "label": {
                "zh_CN": "直播聊天流 ID",
                "en_US": "Live chat stream ID",
                "ja_JP": "ライブチャットストリーム ID",
            },
            "x-widget": "input",
            "advanced": True,
        },
    )
    """直播聊天流 ID；留空时按直播平台自动定位（该平台下必须只有一条聊天流）。"""

    inject_state: bool = Field(
        default=True,
        json_schema_extra={
            "label": {
                "zh_CN": "注入世界状态",
                "en_US": "Inject world state",
                "ja_JP": "ワールド状態を注入",
            },
            "x-widget": "switch",
        },
    ...
    stale_after_seconds: float = Field(
        ...
        json_schema_extra={
            "label": {
                "zh_CN": "状态最长静默间隔",
                "en_US": "Max idle interval for state",
                "ja_JP": "状態の最大静默間隔",
            },
            "x-widget": "number",
            "advanced": True,
    ...
    """同一世界两次普通事件投递之间的最小间隔（秒）。"""

    change_poll_seconds: float = Field(
        default=2.0,
        ge=0.0,
        le=600.0,
        json_schema_extra={
    ...
    """轮询型世界的变更检测周期（秒）；填 0 关闭轮询。"""

    delayed_sources: List[str] = Field(
        default_factory=list,
        json_schema_extra={
            "label": {
                "zh_CN": "防穿帮延迟来源",
                "en_US": "Delayed sources",
                "ja_JP": "遅延ソース",
            },
            "x-widget": "input",
            "advanced": True,
    ...
    """需要防「先知穿帮」延迟的世界机器名列表（如 ["minecraft", "pvz"]）：
    这些世界的事件延迟投递、状态注入时额外给出「观众此刻看到的画面」延迟视图。"""
    ...
    delayed_seconds: int = Field(
        ...
        json_schema_extra={
            "label": {
                "zh_CN": "OBS 画面延迟秒数",
                "en_US": "OBS stream delay seconds",
                "ja_JP": "OBS映像遅延秒数",
            },
            "x-widget": "number",
            "advanced": True,
    ...
    """OBS 画面延迟秒数；delayed_sources 内世界的事件与延迟视图按该秒数对齐。"""
