...
async def startup_event():
    ...
    if use_asr_flag:
        try:
            ...
            # 国内直连 huggingface.co 常因 SSL 证书校验失败，默认走 hf-mirror.com 镜像
            os.environ.setdefault("HF_ENDPOINT", "https://hf-mirror.com")
            
            local_model_path = models_dir / "qwen3_asr"
            repo_id = "Qwen/Qwen3-ASR-0.6B"

            # 兜底查找常见目录结构（含用户手动放置的 API/qwen3_asr/...）
            if not (local_model_path.exists() and (local_model_path / "config.json").exists()):
                for cand in (
                    models_dir / "qwen3_asr" / "Qwen" / "Qwen3-ASR-0.6B",
                    models_dir / "qwen3_asr" / "Qwen3-ASR-0.6B",
                    Path(__file__).parent / "qwen3_asr" / "Qwen" / "Qwen3-ASR-0.6B",
                    Path(__file__).parent / "qwen3_asr",
                ):
