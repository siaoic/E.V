...
class T2SBlock(nn.Module):
    ...
    def decode_next_token(
        ...
        # kv_cache shape [batch_size, kv_len, num_heads, head_dim/num_heads]
        ...
        x = flash_attn_with_kvcache(
            ...
            k=k,
            v=v,
            cache_seqlens=kv_cache_len,
            causal=True,
        )

        x = x.view(B, L, self.hidden_dim)
        x = self.out_proj(x)
...
class T2STransformer(nn.Module):
    ...
    def decode_next_token(
        ...
        kv_cache_len_int32 = kv_cache_len.to(torch.int32)
        for i in range(self.num_blocks):
            x = self.blocks[i].decode_next_token(
                x, k_cache[i], v_cache[i], kv_cache_len_int32
            )
        kv_cache_len += 1
        return x
