...
// ==================== Tab 分组类型与构建 ====================
/**
 * 需要从麦麦设置中隐藏、改由「插件管理 → 内置功能」承载的配置子分区。
 * （语音朗读 TTS 的独立配置页为 /voice-tts。）
 */
const BOT_CONFIG_HIDDEN_SUBSECTIONS: Array<{ section: string; name: string }> = [
  { section: 'chat', name: 'tts_read' },
]

/**
 * 从 schema 中剥离被隐藏的子分区（不可变克隆，不污染 schema 缓存对象），
 * 使对应配置不再出现在设置页的分区/子页签中。
 */
function stripBotConfigHiddenSections(schema: ConfigSchema): ConfigSchema {
  let visible = schema
  for (const { section, name } of BOT_CONFIG_HIDDEN_SUBSECTIONS) {
    const sectionSchema = visible.nested?.[section]
    const sectionNested = sectionSchema?.nested
    if (!sectionSchema || !sectionNested || !(name in sectionNested)) {
      continue
    }
    visible = {
      ...visible,
      nested: {
        ...
        [section]: {
          ...
          nested: Object.fromEntries(
            Object.entries(sectionNested).filter(([key]) => key !== name)
          ),
        },
      },
    }
...
function BotConfigPageContent() {
  ...
  const loadConfig = useCallback(async (): Promise<boolean> => {
    try {
      ...
      if (schemaResult.status === 'fulfilled' && schemaResult.value) {
        ...
        // 剥离由「插件管理 → 内置功能」承载的子分区（如语音朗读），不在设置页显示
        setConfigSchema(schema ? stripBotConfigHiddenSections(schema) : schema)
      }
  ...
  useEffect(() => {
    const hookEntries = [
      ...
      ['bot.platforms', BotPlatformAccountsHook, 'replace'],
