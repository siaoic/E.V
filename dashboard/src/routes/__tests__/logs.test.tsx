...
describe('LogViewerPage 终端面板', () => {
  ...
  it('日期筛选只保留窗口内日志，清除后恢复全部', async () => {
    // 冻结系统时间到 2026-08：日历初始月份跟随"今天"，否则用例会随真实日期腐烂
    vi.useFakeTimers({ now: new Date('2026-08-05T00:00:00'), shouldAdvanceTime: true })
...
describe('LogViewerPage 页签与提示', () => {
  ...
  it('页签切换器在页内工具条中渲染', async () => {
    render(<LogViewerPage />)

    // 切换器不再依赖顶栏 Portal，直接渲染在页面工具条内
    await waitFor(() => {
      expect(document.querySelector('[data-log-viewer-switcher="true"]')).not.toBeNull()
    })
    expect(screen.getByRole('tab', { name: '终端' })).toBeInTheDocument()
    expect(screen.getByRole('tab', { name: '推理过程' })).toBeInTheDocument()
    expect(screen.getByRole('tab', { name: '详细统计' })).toBeInTheDocument()
  })
})
