...
describe('AppearanceTab 主题模式与界面风格', () => {
  ...
  it('界面风格卡片：点击未来复古调用 updateThemeConfig 切换 dashboardStyle', async () => {
    ...
    expect(screen.getByRole('button', { name: /Cortico 纸感/ })).toHaveAttribute(
      'aria-pressed',
      'true'
    )
...
describe('AppearanceTab 样式微调 token 写入', () => {
  ...
  it('字体排版：覆盖值分别映射 serif / mono 选项，缺字号时回落计算默认', async () => {
    const user = userEvent.setup()
    themeState = makeThemeState({
      styleTokenOverrides: {
        modern: {
    ...
    // 覆盖里没有 font-size-base，回落到计算默认 15px（0.9375rem × 16）
    expect(within(region).getByText('15px')).toBeInTheDocument()
  ...
  it('视觉效果：圆角滑块、阴影 Select 与模糊开关写入 visual token', async () => {
    ...
    expect(themeState.updateThemeConfig).toHaveBeenCalledWith({
      styleTokenOverrides: {
        modern: {
          // 默认 0.625rem，右移一格 +0.0625rem
          visual: { 'radius-md': '0.6875rem' },
        },
      },
    })
