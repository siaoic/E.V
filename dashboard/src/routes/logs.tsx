...
function LogTerminalPane({ toolbarContainerId, toolbarVisible }: LogTerminalPaneProps) {
  ...
  const toolbarContent = (
    <Collapsible open={filtersOpen} onOpenChange={handleFiltersOpenChange}>
      {/* 断点用 xl：主区宽度 = 视口 - 侧栏，lg 下双列最小 620px 会与左侧页签切换器重叠 */}
      <div className="grid w-full gap-2 xl:grid-cols-[minmax(0,1fr)_minmax(620px,760px)]">
        <div
          className={cn(
  ...
  return (
    <div className="flex h-full min-h-0 flex-col overflow-hidden">
      ...
      <div className="min-h-0 flex-1 px-2 pb-2 sm:px-3 sm:pb-3 lg:px-4 lg:pt-2 lg:pb-4">
        <Card className="h-full overflow-hidden border-[#292c2b]/70 dark:border-[#292c2b]/80" style={{ backgroundColor: '#1d1f1e' }}>
          <div
            ref={parentRef}
            className={cn(
              'h-full overflow-auto selection:bg-[#3a3d3c]',
              // 自定义滚动条样式
...
export function LogViewerPage({ defaultTab }: LogViewerPageProps) {
  ...
  // 推理过程工具栏是否已接管页内工具区（用于提示条显隐等）
  const [, setReasoningToolbarVisible] = useState(activeTab === 'reasoning')
  ...
  const renderTabSwitcher = (includeTopbarActions = false) => {
    return (
      <div className="flex min-w-0 items-center gap-2">
        <TabsList data-log-viewer-switcher={includeTopbarActions ? 'true' : undefined}>
  ...
  return (
    ...
    >
      {/* 页内工具条：左侧 tab 切换器 + 右侧各 pane 的工具区 */}
      <div
        className={cn(
