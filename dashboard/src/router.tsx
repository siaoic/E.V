...
// 直播语音路由 - B站直播语音朗读（弹幕接入由 world-bilibili 插件负责）
const bilibiliLiveRoute = createRoute({
  getParentRoute: () => protectedRoute,
  path: '/bilibili-live',
  component: lazyRouteComponent(() => import('./routes/bilibili-live'), 'BilibiliLivePage'),
})

// 世界状态路由 - 多 World 框架的只读实时视图（已接入世界 / 状态注入 / 事件流 / VTuber 桥）
const worldsRoute = createRoute({
  getParentRoute: () => protectedRoute,
  path: '/worlds',
  component: lazyRouteComponent(() => import('./routes/worlds'), 'WorldsPage'),
})

// 语音朗读（TTS）独立配置页 - 由插件管理 → 内置功能进入
const voiceTtsRoute = createRoute({
  getParentRoute: () => protectedRoute,
  path: '/voice-tts',
  component: lazyRouteComponent(() => import('./routes/voice-tts'), 'VoiceTtsPage'),
})
